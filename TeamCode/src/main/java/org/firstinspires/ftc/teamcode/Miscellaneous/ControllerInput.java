...
public class ControllerInput {
    ...
    void updateButtons() {
        if(!disableX) { if(gamepad.x) { ++x; } else { x = 0; } }
        if(!disableY) { if (gamepad.y) { ++y; } else { y = 0; } }
        if(!disableA) { if (gamepad.a) { ++a;  } else { a = 0; } }
        if(!disableB) { if (gamepad.b) { ++b; } else { b = 0; } }
    }

    void updateArrows() {
        if (gamepad.dpad_up) { ++dpad_up; return; } else { dpad_up = 0; }
        if (gamepad.dpad_down) { ++dpad_down; return; } else { dpad_down = 0; }
        if (gamepad.dpad_left) { ++dpad_left; return; } else { dpad_left = 0; }
